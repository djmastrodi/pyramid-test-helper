...
class ScanTest {
    constructor() { }
    readFileContent(filepath, regex, regexExclude) {
    ...
    scanFileTest(directory, fileNameRegex, regexCount, regexExclude) {
        let count = 0;
        fs.readdirSync(directory).forEach((file) => {
            const stat = fs.statSync(`${directory}/${file}`);
            if (stat.isDirectory()) {
                count += this.scanFileTest(`${directory}/${file}`, fileNameRegex, regexCount, regexExclude);
            }
